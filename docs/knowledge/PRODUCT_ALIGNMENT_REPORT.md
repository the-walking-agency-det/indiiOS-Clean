...
> **Goal:** Ensure the IndiiOS platform features directly capture the value identified in the _Artist Economics Deep Dive_.
...
- **The Execution Gap:**
  - [ ] **Feature:** **"Golden File" System**. When a song is finished in `MusicStudio`, it MUST require immediate population of ISRC, ISWC, Songwriter Splits, and Publisher info _before_ it can be exported or distributed.
...
- **The Execution Gap:**
  - [ ] **Feature:** **The "Hype Engine".** `PublicistAgent` needs to read the `Music_Industry_History_Deep_Dive` to understand _context_ and write press releases that actually fit the narrative.
