...
- **Core Philosophy**: Procedural animation. Instead of defining state at frame X, you define _flows_ using generator functions (e.g., `yield* circle().position(100, 1).to(0, 1)`).
...
2. **UI Components**: Use **Twick** (or similar) for the _Timeline UI_ logic if you don't want to build drag-and-drop tracks from scratch.
