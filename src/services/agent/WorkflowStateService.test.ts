    ...
    describe('advanceStep', () => {
        it('should mark a step as complete and advance the index', async () => {
            const storedExecution: WorkflowExecution = {
                ...
                currentStepIndex: 0,
                steps: [
                    { stepIndex: 0, agentId: 'brand', prompt: 'Analyze brand', status: 'executing', startedAt: 1000 },
                    { stepIndex: 1, agentId: 'marketing', prompt: 'Create strategy', status: 'planned' },
                    { stepIndex: 2, agentId: 'social', prompt: 'Draft posts', status: 'planned' },
                ],
                createdAt: 1000,
                updatedAt: 1000,
            };
        ...
        it('should mark the workflow as completed when all steps are done', async () => {
            const storedExecution: WorkflowExecution = {
                ...
                currentStepIndex: 2,
                steps: [
                    { stepIndex: 0, agentId: 'brand', prompt: 'Analyze brand', status: 'step_complete', result: 'Done' },
                    { stepIndex: 1, agentId: 'marketing', prompt: 'Create strategy', status: 'step_complete', result: 'Done' },
                    { stepIndex: 2, agentId: 'social', prompt: 'Draft posts', status: 'executing', startedAt: 2000 },
                ],
                createdAt: 1000,
                updatedAt: 2000,
            };
    ...
    describe('failStep', () => {
        it('should mark a step and the workflow as failed while preserving remaining planned steps', async () => {
            const storedExecution: WorkflowExecution = {
                ...
                steps: [
                    { stepIndex: 0, agentId: 'brand', prompt: 'Analyze brand', status: 'step_complete', result: 'Done' },
                    { stepIndex: 1, agentId: 'marketing', prompt: 'Create strategy', status: 'executing', startedAt: 1500 },
                    { stepIndex: 2, agentId: 'social', prompt: 'Draft posts', status: 'planned' },
                ],
                createdAt: 1000,
                updatedAt: 1500,
