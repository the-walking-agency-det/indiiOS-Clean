...
const PRIVATE_HOSTNAMES = /^(localhost|127\.\d+\.\d+\.\d+|0\.0\.0\.0|10\.\d+\.\d+\.\d+|172\.(1[6-9]|2\d|3[01])\.\d+\.\d+|192\.168\.\d+\.\d+|169\.254\.\d+\.\d+|\[::1\]|::1)$/i;

export function isDeepLinkSafe(url: string): boolean {
    try {
        ...
        if (!/^[a-zA-Z0-9\-_/?.=&% ]*$/.test(fullPath)) return false;
        return true;
    } catch {
        return false;
    }
