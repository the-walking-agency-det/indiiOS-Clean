...
exports.exportSplitSheet = void 0;
/**
...
const CollaboratorSchema = zod_1.z.object({
    ...
    splitPct: zod_1.z.number().min(0).max(100),
    email: zod_1.z.string().email().optional(),
    signed: zod_1.z.boolean().optional().default(false),
    signedAt: zod_1.z.string().optional(),
});
const ExportSplitSheetSchema = zod_1.z.object({
    splitSheetId: zod_1.z.string().min(1),
    releaseTitle: zod_1.z.string().min(1),
    releaseIsrc: zod_1.z.string().optional(),
    upc: zod_1.z.string().optional(),
    collaborators: zod_1.z.array(CollaboratorSchema).min(1),
    totalEscrowAmount: zod_1.z.number().min(0).optional(),
    effectiveDate: zod_1.z.string().optional(),
    notes: zod_1.z.string().optional(),
});
/* ── HTML Template ───────────────────────────────────────────────────────── */
function buildSplitSheetHtml(data, generatedAt) {
...
/* ── Cloud Function ──────────────────────────────────────────────────────── */
exports.exportSplitSheet = (0, https_1.onCall)({ cors: true, enforceAppCheck: true }, async (request) => {
    var _a;
    ...
    if (!parseResult.success) {
        throw new https_1.HttpsError('invalid-argument', `Invalid split sheet data: ${parseResult.error.message}`);
    }
...
//# sourceMappingURL=exportSplitSheet.js.map