...
exports.pollTimelineMilestones = void 0;
...
// ============================================================================
let inngestClient = null;
function getInngest() {
    if (!inngestClient) {
        const eventKey = process.env.INNGEST_EVENT_KEY || '';
        inngestClient = new inngest_1.Inngest({
 ...
 */
exports.pollTimelineMilestones = (0, scheduler_1.onSchedule)({
    ...
    try {
            ...
            for (const timelineDoc of activeTimelinesSnap.docs) {
                ...
                for (let i = 0; i < timeline.milestones.length; i++) {
                    ...
                    if (milestone.status === 'pending' && milestone.scheduledAt <= now) {
                        ...
                        timeline.milestones[i] = Object.assign(Object.assign({}, milestone), { status: 'executing' });
                        updated = true;
                        console.log(`[pollTimelineMilestones] Dispatching milestone: "${milestone.instruction.slice(0, 80)}..." ` +
                        ...
                        try {
                            await inngest.send({
                                ...
                                data: {
                                    ...
                                    type: milestone.type,
                                    platform: (_a = milestone.platform) !== null && _a !== void 0 ? _a : null,
                                    goal: timeline.goal,
                            ...
                            totalProcessed++;
                            console.log(`[pollTimelineMilestones] Inngest event sent for milestone ${milestone.id}`);
                        }
                        catch (sendError) {
                            console.error(`[pollTimelineMilestones] Failed to send Inngest event for milestone ${milestone.id}:`, sendError);
                ...
                if (updated) {
                    const dueMilestoneIds = new Set(timeline.milestones
                    ...
                    await db.runTransaction(async (transaction) => {
                        ...
                        if (!freshSnap.exists)
                            return;
                        const freshTimeline = freshSnap.data();
                        ...
                        const completedCount = freshMilestones.filter((m) => m.status === 'completed').length;
                        const allDone = freshMilestones.every((m) => m.status === 'completed' || m.status === 'skipped' || m.status === 'failed');
                        const updates = {
        ...
        console.log(`[pollTimelineMilestones] Done. Found ${totalDue} due milestones, dispatched ${totalProcessed} to Inngest.`);
    }
    catch (error) {
        console.error('[pollTimelineMilestones] Fatal error:', error);
    }
});
//# sourceMappingURL=pollTimelineMilestones.js.map