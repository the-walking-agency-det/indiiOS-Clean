...
export class IdentifierService {
    ...
    private static readonly INITIAL_ISRC_SEQUENCE = 100;
    private static readonly INITIAL_UPC_SEQUENCE = 10000000000;
    private static readonly DEFAULT_ISRC_YEAR = 26;
    ...
    private static async getNextSequence(type: 'isrc' | 'upc', year?: number): Promise<number> {
        ...
        return await runTransaction(db, async (transaction) => {
            ...
            if (!seqDoc.exists()) {
                ...
                const initialData = {
                    isrc: { lastYear: year || IdentifierService.DEFAULT_ISRC_YEAR, sequence: IdentifierService.INITIAL_ISRC_SEQUENCE }, // Start at padding baseline
                    upc: { sequence: IdentifierService.INITIAL_UPC_SEQUENCE } // 11 digits start
                };
            ...
            if (type === 'isrc') {
                const isrcData = data.isrc || { lastYear: year || IdentifierService.DEFAULT_ISRC_YEAR, sequence: IdentifierService.INITIAL_ISRC_SEQUENCE };
                ...
                const upcData = data.upc || { sequence: IdentifierService.INITIAL_UPC_SEQUENCE };
