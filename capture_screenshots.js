...
(async () => {
  ...
  for (const { route, filename } of routes) {
    ...
    // Hash routing is often used in Electron apps, if it's not hash router, use /route
    // Let's try both if it fails, but I'll use /route first, wait, is it browser router or hash router?
    // Let's just use /route. If it's a SPA, Vite handles it, but if it's Electron, it might be HashRouter.
    await page.goto(`http://localhost:4242/${route}`);
    await page.waitForTimeout(4000); // Wait for animations/loading
