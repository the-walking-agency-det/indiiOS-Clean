...
class SchedulerTaskList(BaseModel):
    ...
    def get(cls) -> "SchedulerTaskList":
        ...
        if cls.__instance is None:
            if not exists(path):
                make_dirs(path)
                # Ensure we create it synchronously to avoid asyncio.run nesting errors
                instance = cls(tasks=[])
            ...
            if exists(path):
                with cls.__instance._lock:
                    data = cls.model_validate_json(read_file(path))
                    cls.__instance.tasks.clear()
                    cls.__instance.tasks.extend(data.tasks)
