...
**Prevention:** Always use `fs.realpathSync` (or async equivalent) to resolve the canonical path _before_ applying blocklists or allowlists (extensions, directories). This ensures validation is performed on the actual file being accessed.
...
1. Always use `validateSafeUrlAsync(url)` (or equivalent DNS-resolving validator) _before_ making network requests in privileged contexts (Electron Main process).
...
**Learning:** Logging raw inputs for "deep diagnosis" is a common trap. While helpful for debugging, it bypasses downstream sanitization logic. Security-critical sanitization must occur _before_ any data crosses a trust boundary, including writing to logs.
**Prevention:** Modified `FirebaseAIService.ts` to explicitly call `sanitizePrompt` on the input payload _before_ passing it to `console.log`. This ensures that PII is redacted in the logs while still providing useful debug information.
