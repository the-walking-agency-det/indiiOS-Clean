...
**Learning:** The "How can I help you?" placeholder in `ChatOverlay.tsx` only disappears when `messages.length > 0`. Waiting for it to detach _before_ adding a message causes a timeout. Tests must verify its presence first, _then_ add a message to trigger its removal.
...
**Action:** When testing container components, verify dependencies of _all_ children (even docked ones like `PromptArea`) and include them in the mock store/context.
