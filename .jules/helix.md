...
## 2026-02-17 - [Empty Soul Prevention]
**Learning:** LLMs occasionally return empty strings or whitespace-only responses when mutating prompts (e.g., due to silent refusals or context failures). This results in "Soul-less" agents with no instructions.
**Action:** Verified that the `EvolutionEngine` explicitly checks for `!systemPrompt || systemPrompt.trim() === ''` and rejects these mutations. Added `HelixEmptySoul.test.ts` to enforce this guardrail permanently.
