...
def audit_fidelity(file_path: str, target: str = "Hi-Res") -> Dict[str, Any]:
    ...
    try:
        ...
        audio_stream = next(
            (s for s in metadata["streams"] if s.get("codec_type") == "audio"),
            None
        )
        if not audio_stream:
        ...
        # Handle cases where bit depth might be missing (e.g., lossy files or
        # some containers)
        bit_depth = int(
            audio_stream.get(
                "bits_per_sample",
        ...
        # Atmos check
        is_atmos = channels >= 6 or (
            format_name == "asf" and "atmos" in str(metadata).lower()
        )
        ...
        if t_low == "hi-res":
            if not report["compliance"]["Hi_Res"]:
                report["summary_status"] = (
                    "FAIL (Target: Hi-Res | Current specs do not "
                    "meet 96kHz/24-bit minimum)"
                )
        elif t_low == "atmos":
            if not report["compliance"]["Atmos_Ready"]:
                report["summary_status"] = (
                    "FAIL (Target: Atmos | Insufficient channels "
                    "or incompatible format)"
                )
        elif t_low == "cd":
            if not report["compliance"]["CD_Quality"]:
                report["summary_status"] = (
                    "FAIL (Target: CD | Current specs do not "
                    "meet 44.1kHz/16-bit minimum)"
                )
        ...
        logger.error(
            f"Data parsing error during fidelity audit for {file_path}: {e}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(json.dumps(
            {"error": "No file path provided. Usage: python3 audio_fidelity_audit.py <path> [target]"}))
        sys.exit(1)
