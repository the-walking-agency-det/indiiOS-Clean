...
try:
    ...
    logger.warning("librosa not installed. Spectral analysis will be disabled.")
    librosa = None
    ...
    try:
        ...
        if high_energy < cutoff_threshold:
            return {
                ...
                "details": f"Mean energy above 16kHz is {high_energy:.6f}, which is below the threshold of {cutoff_threshold}"
            }
        
        return {
            "status": "PASS", 
            "high_freq_energy": high_energy,
            "details": f"Healthy energy levels detected in high-frequency range (>16kHz)."
        }
    ...
    try:
        ...
        if clipping_percentage > 0.01:  # More than 0.01% samples clipped
            return {
                ...
                "details": f"Found {len(clipped_samples)} clipped samples. Audio may be distorted."
            }
...
def detect_silence(file_path: str, top_db: int = 60) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "details": f"Detected {start_silence:.2f}s leading and {end_silence:.2f}s trailing silence."
        }
    except Exception as e:
...
def audit_audio(file_path: str) -> Dict[str, Any]:
    ...
    # 2. Clipping Check
    report["forensics"]["clipping"] = detect_clipping(file_path)
    
    # 3. Silence Check
    report["forensics"]["silence"] = detect_silence(file_path)

    # Determine summary status
    forensic_results = report["forensics"].values()
    if any("FAIL" in str(r.get("status", "")) for r in forensic_results):
        report["summary_status"] = "FAIL (Audio Quality Fraud Detected)"
    elif any("WARNING" in str(r.get("status", "")) for r in forensic_results):
        report["summary_status"] = "WARNING (Quality Issues Detected)"

    return report

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(json.dumps({"error": "No file path provided. Usage: python3 audio_forensics.py <path>"}))
        sys.exit(1)
