...
def test_v3_compliance():
    ...
    assert_test("Fraud Certification Lock", cert['payout_status'] == "HELD", f"Status: {cert['payout_status']}")
    
    payout = engine.calculate_withholding("user_fraud_123", 1000.0)
    assert_test("Payout Lock Verification", payout['status'] == "HELD" and payout['payable_amount'] == 0, f"Payable: {payout['payable_amount']}")
    ...
    assert_test("Unsigned Perjury Lock", cert_2['payout_status'] == "HELD", f"Certified: {cert_2['certified']}")
    
    payout_2 = engine.calculate_withholding("user_unsigned_456", 1000.0)
    assert_test("Unsigned Payout Reason", "Missing certification" in payout_2['reason'], payout_2['reason'])
    ...
    assert_test("UK Treaty Application", payout_3['status'] == "ACTIVE" and payout_3['withholding_rate'] == 0, f"Rate: {payout_3['withholding_rate']}%")
