    ...
    def generate_bwarm_csv(self, works: List[Dict[str, Any]]) -> str:
        ...
        for work in works:
            writer.writerow({
                ...
                'Original Release Date': work.get('release_date', datetime.datetime.now().strftime('%Y-%m-%d'))
            })
            
        return output.getvalue()

    def check_merlin_compliance(self, catalog_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Merlin typically requires a catalog size of ~50+ tracks or significant revenue
        if track_count >= 50:
            score += 40
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(json.dumps({"error": "Usage: keys_manager.py [bwarm|merlin_check] <json_payload>"}))
        sys.exit(1)
